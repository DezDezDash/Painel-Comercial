...
def wait_ajax(driver, timeout=15):
    """Aguarda AJAX do PrimeFaces/JSF terminar."""
    try:
...
def select_pf_dropdown(driver, label_text, option_text):
    try:
        ...
        item = panel.find_element(By.XPATH, f".//li[normalize-space(text())='{option_text}']")
        driver.execute_script("arguments[0].click();", item)
        wait_ajax(driver)
        return True
    except Exception as e:
        print(f"    [pf_dropdown fallback] {label_text}={option_text}: {e}")
    ...
    try:
        ...
        print(f"    [select fallback] {label_text}={option_text}: {e2}")
    return False
...
def set_date_field(driver, date_str, is_end=False):
    try:
        ...
        if not date_inputs:
            all_text = driver.find_elements(By.XPATH, "//input[@type='text']")
            date_inputs = [
                i for i in all_text
        ...
        print(f"    [set_date {'fim' if is_end else 'inicio'}] {e}")
    return False
...
def navigate_to_painel_vendas(driver):
    ...
    try:
        ...
        time.sleep(1)
        driver.find_element(By.XPATH, "//*[contains(text(),'Painel de Vendas') or contains(@title,'Painel de Vendas')]").click()
        wait_ajax(driver)
        time.sleep(2)
...
def run_extraction(driver, loja_code, loja_label, por_option, start_str, end_str):
    ...
    try:
        ...
        btn = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(.,'Pesquisar')] | //a[contains(.,'Pesquisar')]"))
        )
...
def main():
    ...
    for loja in LOJAS:
        ...
        total = sum(vend.values())
        print(f"  {loja}: R$ {total:,.2f}  ({len(vend)} vendedores)")
